...
class Player(BaseModel):
    ...
    def get_buyable_cards(self, cards: list[Card]) -> list[Card]:
        ...
        buyable_cards = [
            card
            for card in cards
            if self.tokens[Token.YELLOW]
            >= self.calculate_real_price(card)[Token.YELLOW]
        ]
        return buyable_cards

    def get_token_collection_moves(
        self, available_tokens: dict[Token, int]
    ) -> list[dict[Token, int]]:
        ...
        collectable_tokens = {
            token: num for token, num in available_tokens_copy.items() if num > 0
        }
        token_collection_moves = []
        # 2 of same type
        token_collection_moves = [
            {token: 2}
            for token, available in collectable_tokens.items()
            if available >= 4
        ]
        # 1 of three different types
        token_collection_moves += [
            {token: 1 for token in combo}
            for combo in combinations(collectable_tokens, 3)
        ]
        return token_collection_moves

    def get_possible_moves(
        self,
        available_tokens: dict[Token, int],
        ...
        buyable_cards = self.get_buyable_cards(
            available_cards
        ) + self.get_buyable_cards(self.reserved_cards)
        card_real_price_cache = [
            self.calculate_real_price(card) for card in buyable_cards
        ]
        buy_card_moves = [
            {"move_type": "buy_card", "card": buyable_cards[i], "payment": real_price}
            for i, real_price in enumerate(card_real_price_cache)
        ]
        if not reduced:
            buy_card_moves += [
                {"move_type": "buy_card", "card": buyable_cards[i], "payment": payment}
                for i, real_price in enumerate(card_real_price_cache)
                for payment in self.get_payment_combinations(
                    real_price, self.tokens[Token.YELLOW] - real_price[Token.YELLOW]
                )
            ]
        unique_moves = []
        [
            unique_moves.append(move)
            for move in buy_card_moves
            if move not in unique_moves
        ]
        buy_card_moves = unique_moves

        reservable_cards = (
            available_cards if len(self.reserved_cards) < 3 else []
        )  # + 3 face down cards
        returnable_tokens = (
            [{token: 1} for token in self.tokens if self.tokens[token] > 0]
            if sum(self.tokens.values()) + 1 > 10
            else [{}]
        )
        reserve_card_moves = [
            {"move_type": "reserve_card", "card": card, "returning": returning}
            for card in reservable_cards
            for returning in returnable_tokens
        ]

        collectable_tokens = self.get_token_collection_moves(available_tokens)
        collection_moves = [
            {"move_type": "collect_tokens", "tokens": tokens, "returning": returning}
            for tokens in collectable_tokens
            for returning in self.get_possible_tokens_to_return(
    ...
    def get_possible_tokens_to_return(
        self, additional_tokens: dict[Token, int] = {}
    ) -> list[dict[Token, int]]:
        ...
        if num_to_return <= 0:
            return [{}]
        tokens_flat_list = [
            token for token, amount in tokens.items() for _ in range(amount)
        ]
        valid_combinations_tuples = set(
            list(combinations(tokens_flat_list, r=num_to_return))
        )
        valid_combinations = [
    ...
    def calculate_real_price(self, card: Card) -> dict[Token, int]:
        ...
        real_price = {
            token: max((card.price[token] - self.bonuses[token]), 0)
    ...
    def get_payment_combinations(
        self, tokens: dict[Token, int], num_yellows: int
    ) -> list[dict[Token, int]]:
        ...
        tokens_flat_list = [
            token for token, amount in tokens_copy.items() for _ in range(amount)
        ]
        unique_combos = []
