...
def expansion(
    current_node: Node, sample_size: int = -1, weights: list[int] = [1, 1, 1]
) -> Node:
    ...
    possible_moves = current_node.gamestate.current_player.get_possible_moves(
        available_tokens, available_cards, reduced=config["reduced"]
    )
    if sample_size > 0:
...
def rollout(current_node: Node, pov: Player) -> Node:
    ...
    while not game.is_over():
        for _ in game.players:
            # Select move
            try:
                all_moves = game.current_player.get_possible_moves(
                    game.board.available_tokens,
                    game.board.available_cards,
...
def back_propagate_rave(
    current_node: Node, terminal_value: int, rave_moves: list[dict]
) -> Node:
    rollout_node = current_node
...
def mcts_rave(current_node: Node, immediate_moves: list[dict]) -> Node:
    ...
    # rollout
    terminal_node = rollout_rave(
        next_node, current_node.gamestate.current_player, immediate_moves
    )

    # backpropagation
    next_node = back_propagate_rave(
        next_node, terminal_node.value, terminal_node.children
    )
    return current_node


def select_move_with_mcts(
    current_node: Node, mcts_budget: int, enhancement: str = None
):
    ...
    if config["time_limit"]:
        ...
        while datetime.utcnow() - start_time < timedelta(seconds=mcts_budget):
            current_node = (
                mcts(current_node)
                if enhancement == None
                else mcts_rave(current_node, immediate_moves)
            )
...
def sample_moves(
    all_moves: list[dict], k: int, weights: list[int] = [1, 1, 1]
) -> list[dict]:
    ...
    while len(unique_sampled_moves) < k:
        sampled_moves = random.choices(
            all_moves, weights=w, k=(k - len(unique_sampled_moves))
        )
        for move in sampled_moves:
