...
from prbx_project.simulation.all_cards import all_cards
...
class Board(BaseModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.available_cards: list[Card] = [
            self.all_cards[tier].pop(random.randrange(len(self.all_cards[tier])))
            for _ in range(4)
            for tier in range(3)
        ]  # randomly select 4 cards from each tier of cards
        ...
        for token, amount in tokens.items():
            ...
            if token in self.available_tokens:
                if amount > self.available_tokens[token]:
                    raise ValueError(
                        "Board cannot remove more tokens than are present."
                    )
