...
class GameState(BaseModel):
    ...
    def get_winner(self) -> Player:
        ...
        if player1.points < 15 and player2.points < 15:
            raise ValueError(
                f"Game has not finished, no players have reached {self.max_points} points"
            )
    ...
    def replace_card(self, board: Board, card: Card, log: bool = False) -> Card | None:
        ...
        try:
            ...
            if log:
                print(
                    f"No more tier {card.tier} cards in the deck, could not replace.",
                    end=" ",
                )
            return None
    ...
    def play_move(self, move: dict, log: bool = False):
        match move["move_type"]:
            case "buy_card":
                card: Card = move["card"]
                self.buy_card(
                    self.current_player, self.board, card, move["payment"], log
                )
            case "reserve_card":
                card: Card = move["card"]
                self.reserve_card(
                    self.current_player, self.board, card, move["returning"], log
                )
            case "collect_tokens":
                tokens: dict[Token, int] = move["tokens"]
                self.collect_tokens(
                    self.current_player, self.board, tokens, move["returning"]
                )
        # self.next_player()
        return self
    ...
    def num_tokens_in_play(
        self, board: Board, player1: Player, player2: Player
    ) -> tuple[dict[Token, int], int]:
